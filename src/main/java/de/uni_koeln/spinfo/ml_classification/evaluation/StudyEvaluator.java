...
//	private double coverage = 0d;
	...
	public void evaluate(Map<ClassifyUnit, Map<String, Boolean>> classified, List<String> studyList) {
		...
		for (int i = 0; i < numberOfClasses; i++) {
			classesList.add(studyList.get(i));
		}
		evaluate(classified, classesList, studyList);
	}
...
//			List<Entry<String, Double>> rankedLabels = Util.sortByComparator(((FocusClassifyUnit) cu).getRanking());
			...
			int predictLabels = 0;
			for (int i = 0; i < numberOfClasses; i++) {
				String currentLabel = studyList.get(i);
...
//						printFP++;
//						if(printFP < 12){
//							System.out.println(categoryResults.get(i).getLabel());
